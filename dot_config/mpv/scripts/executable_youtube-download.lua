...
-- Read proxy from mpv.conf automatically
local mpv_http_proxy = mp.get_property("options/http-proxy")

local function inject_proxy(command)
    if mpv_http_proxy ~= nil and mpv_http_proxy ~= "" then
        table.insert(command, "--proxy")
        table.insert(command, mpv_http_proxy)
    end
end
