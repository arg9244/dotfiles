...
function Timeline:init()
	...
	-- Clear any previous heatmap and reset flag on file load
	...
	-- Load heatmap when duration becomes known
	self:observe_mp_property("duration", function(_, dur)
		if not self.heatmap_loaded and dur and dur > 0 then
			self.heatmap = load_embedded_heatmap() or load_youtube_heatmap()
			self.heatmap_loaded = true
			request_render()
		end
	end)

	-- Also try when metadata changes (in case duration was already there)
	self:observe_mp_property("metadata", function()
		if not self.heatmap_loaded and state.duration and state.duration > 0 then
			self.heatmap = load_embedded_heatmap() or load_youtube_heatmap()
			self.heatmap_loaded = true
			request_render()
		end
