...
-- Load heatmap embedded in local file's metadata.
-- Expects a JSON array of objects: [{"start_time":0,"end_time":20.6,"value":0.317}, ...]
function load_embedded_heatmap()
    local metadata = mp.get_property_native("metadata") or {}
    -- Matroska tags are case‑sensitive; try both common cases
    local heatmap_str = metadata["heatmap"] or metadata["HEATMAP"]
    if not heatmap_str or heatmap_str == "" then
        return nil
    end

    local ok, data = pcall(utils.parse_json, heatmap_str)
    if not ok or type(data) ~= "table" or #data == 0 then
        msg.warn("Invalid heatmap JSON in metadata")
        return nil
    end

    local duration = state.duration
    if not duration or duration <= 0 then
        msg.warn("Cannot load heatmap: duration unknown")
        return nil
    end

    -- Normalise values (same as YouTube heatmap)
    local max_val = 0
    for _, seg in ipairs(data) do
        if seg.value and seg.value > max_val then max_val = seg.value end
    end
    if max_val == 0 then return nil end

    -- Build normalised points list (bottom‑left to top‑right)
    local norm = {0, 1}   -- start at (0,1)
    for i, seg in ipairs(data) do
        local start_time = seg.start_time
        local end_time = (i < #data) and data[i+1].start_time or duration
        local center = (start_time + end_time) / 2
        local norm_x = center / duration
        local norm_y = 1 - (seg.value / max_val)
        table.insert(norm, norm_x)
        table.insert(norm, norm_y)
    end
    table.insert(norm, 1)   -- final x
    table.insert(norm, 1)   -- final y
