#!/usr/bin/env python3
"""Linrar — Universal RAR Backup & Restore (GTK4 Background Analyzer)
...
def estimate_rar_uncompressed_size(first_vol: str, total_comp_bytes: int, password: str = "") -> int:
    ...
    try:
        res = subprocess.run(
            [rar_bin, "v", "-v", "-c-"] + ([f"-p{password}"] if password else []) + [first_vol],
            capture_output=True,
            text=True,
            timeout=6.0
        )
        ...
        for line in reversed(lines):
            # RAR v -v summary row (last line): "<uncomp_total> <packed_total> <ratio>% [<volumes>]"
            parts = line.split()
            if len(parts) >= 3 and parts[0].isdigit() and parts[1].isdigit() and parts[2].rstrip('%').isdigit():
                uncomp = int(parts[0])
                if uncomp > 0:
                    return uncomp
...
class RarBackupWindow(Gtk.ApplicationWindow):
    def __init__(self, app):
        super().__init__(application=app, title="Linrar — Gruvbox")
    ...
    def _build_header(self, parent):
        ...
        title = Gtk.Label()
        title.set_markup("<span size='medium' weight='bold' foreground='#fabd2f'>📦 Linrar</span>")
        hbox.append(title)
    ...
    def _build_tab_compression(self):
        ...
        # Password Encryption
        self._enc_check = Gtk.CheckButton(label="Encrypt (-hp):")
        self._enc_check.set_tooltip_text("Encrypts archive data AND file headers with a password (RAR -hp switch).")
        self._enc_check.connect("toggled", lambda w: self._pass_entry.set_sensitive(w.get_active()))
        grid_params.attach(self._enc_check, 0, 5, 1, 1)

        self._pass_entry = Gtk.Entry(hexpand=True, sensitive=False)
        self._pass_entry.set_visibility(False)
        self._pass_entry.set_placeholder_text("Archive password...")
        self._pass_entry.set_tooltip_text("Used at run time only — never saved to settings.json.")
        grid_params.attach(self._pass_entry, 1, 5, 3, 1)
    ...
    def _build_tab_extraction(self):
        ...
        grid.attach(Gtk.Label(label="Password:"), 0, 5, 1, 1)
        self._ext_pass_entry = Gtk.Entry(hexpand=True)
        self._ext_pass_entry.set_visibility(False)
        self._ext_pass_entry.set_placeholder_text("(empty for unprotected archives)")
        self._ext_pass_entry.set_tooltip_text("Password for -p/-hp protected archives. Run time only — never saved.")
        grid.attach(self._ext_pass_entry, 1, 5, 2, 1)
    ...
    def _run_compression(self):
        ...
        comp_password = self._pass_entry.get_text()
        if self._enc_check.get_active() and comp_password:
            rar_cmd.append(f"-hp{comp_password}")
            self._log("🔒 Header + data encryption enabled (-hp)\n")
    ...
    def _run_extraction(self):
        ...
        ext_password = self._ext_pass_entry.get_text().strip()
        ...
        target_bytes = estimate_rar_uncompressed_size(first_vol, total_comp_bytes, ext_password)
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            rar_cmd = [
                ...
                *( [f"-p{ext_password}"] if ext_password else [] ),
                f"-w{work_dir}",
                "-idq",
                first_vol,
                dest_dir + os.sep
...
_DESKTOP_ID = "linrar.desktop"
_ICON_NAME = "linrar"

_ICON_SVG = """<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
  <!-- Gruvbox archive box with lid + zipper pull -->
  <path d="M14 56 Q14 30 40 30 L88 30 Q114 30 114 56 Z" fill="#fe8019"/>
  <rect x="14" y="50" width="100" height="66" rx="10" fill="#3c3836" stroke="#fabd2f" stroke-width="5"/>
  <rect x="58" y="56" width="12" height="54" rx="5" fill="#fabd2f"/>
  <rect x="55" y="102" width="18" height="12" rx="4" fill="#b8bb26"/>
</svg>
"""


def _is_terminal_run() -> bool:
...
def ensure_self_install():
    ...
    try:
        ...
        entry = (
            ...
            "Version=1.0\n"
            "Name=Linrar\n"
            "GenericName=RAR Archive Backup & Restore\n"
            "Comment=RAR compression/extraction with recovery records, .rev reconstruction and BLAKE3\n"
            f"Exec={exec_line}\n"
            f"Icon={_ICON_NAME}\n"
            "Terminal=false\n"
            "Categories=Utility;Archiving;GTK;\n"
            "Keywords=rar;archive;backup;compress;extract;unrar;\n"
            f"StartupWMClass=com.github.rar_backup_pro\n"
        )
